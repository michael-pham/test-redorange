...
def _render_inputs_html(model):
    inputs = []
    form_model_name = "new" + model.name
    ...
    for attribute in model.attributes:
        ...
        if attribute.ui_type == DATE_INPUT:
            data = input(form_model_name, attribute)
            data[TPL_PATH_KEY] = PATH_BASE + DATE_INPUT_HTML_TPL
            inputs.append(data)
...
def render_item_create_modal_html(model):
    data = dict()
    data[TPL_PATH_KEY] = PATH_BASE + ITEM_CREATE_MODAL_HTML_TPL
